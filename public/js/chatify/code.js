  ...
  if (messenger != 0) {
    ...
    $.ajax({
      ...
      success: (data) => {
        ...
        var userImage = (data.user_avatar === '') ? 
        window.assetUrl.defaultAvatar : 
        window.assetUrl.base + "/" + data.user_avatar;

        // avatar photo
        $(".messenger-infoView")
          .find(".avatar")
          .css("background-image", 'url("' + userImage + '")');
        $(".header-avatar").css(
          "background-image",
          'url("' + userImage + '")'
        );
        ...
        messageInput.focus();
        // update info in view
        $(".messenger-infoView .info-name").text(data.fetch.fname + " " + data.fetch.lname);
        $(".m-header-messaging .user-name").text(data.fetch.fname + " " + data.fetch.lname);
        // Star status
        data.favorite > 0
